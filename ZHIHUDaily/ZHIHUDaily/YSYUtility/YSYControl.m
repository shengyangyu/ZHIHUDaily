...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_touchBlock) {
        _touchBlock(self, YSYGestureRecognizerStateBegan, touches, event);
    }
    if (_longPressBlock) {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_touchBlock) {
        _touchBlock(self, YSYGestureRecognizerStateMoved, touches, event);
    }
    [self endTimer];
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    if (_longPressDetected) return;
    if (_touchBlock) {
        _touchBlock(self, YSYGestureRecognizerStateEnded, touches, event);
    }
    [self endTimer];
}
...
- (void)touchesCancelled:(NSSet *)touches withEvent:(UIEvent *)event {
    if (_longPressDetected) return;
    if (_touchBlock) {
        _touchBlock(self, YSYGestureRecognizerStateCancelled, touches, event);
    }
    [self endTimer];
}
